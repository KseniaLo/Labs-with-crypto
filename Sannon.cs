...
namespace Lab_4
{
    class Sannon
    {
        ...
        // Lengths of the code-words.
        private List<int> Lengths(List<double> P, List<int> L)
        {
            L = new List<int>();
            for (int i = 0; i < P.Count; i++)
                L.Add((int)Math.Ceiling(-Math.Log(P[i], 2)));
            return L;
        }

        // Calculate of the entropy of the text.
        private void H(RichTextBox rtb, List<double> P)
        {
            double sum = 0;
            for (int i = 0; i < P.Count; i++)
            ...
            rtb.AppendText("Энтропия исходного текста = " + Math.Round(sum, 3).ToString()+"\n\r");
        }
        ...
        private void AverageLength(List<double> P, List<int> L, RichTextBox rtb)
        {
            ...
            rtb.AppendText("Средняя длина по Шеннону = " + Math.Round(sum, 3).ToString() + "\n\r");
        }
